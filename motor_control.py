# =============================================================================
# MOTOR CONTROL - FROM OLD CODE (EXACT PRESERVATION)
# This file preserves the reliable, lag-free motor communication from old code
# =============================================================================
...
class MotorControl:
    def __init__(self):
        self.ser = None  # FROM OLD CODE - proper initialization
        self.crc8 = crcmod.predefined.mkPredefinedCrcFun("crc-8-maxim")
        self.prev_fb_rpm = [0, 0]
        self.prev_fb_cur = [0, 0]
        self.connect()  # FROM OLD CODE - connect on initialization

    def connect(self):
        """Connect to motor controller - FROM OLD CODE (EXACT PRESERVATION)"""
        try:
    ...
    def close(self):
        """Close motor connection - FROM OLD CODE (EXACT PRESERVATION)"""
        if self.ser and self.ser.is_open:
            self.ser.close()
            logger.info("Motor serial port closed")

    def send_rpm(self, motor_id, rpm):
        """Send RPM command to motor - FROM OLD CODE (EXACT PRESERVATION)"""
    ...
    def set_drive_mode(self, motor_id, mode):
        """Set drive mode for motor - FROM OLD CODE (EXACT PRESERVATION)"""
        if not self.ser or not self.ser.is_open:
            logger.error("Motor serial port not open")
            return False
    ...
    def get_feedback(self, motor_id):
        """Get feedback from motor - FROM OLD CODE (EXACT PRESERVATION)"""
        if not self.ser or not self.ser.is_open:
    ...
    def _crc_attach(self, data_bytes):
        """Attach CRC to data - FROM OLD CODE (EXACT PRESERVATION)"""
        crc_int = self.crc8(data_bytes)
        return data_bytes + bytes([crc_int])

    def _current_raw_to_amp(self, cur_raw):
        """Convert raw current value to amperes - FROM OLD CODE (EXACT PRESERVATION)"""
        return (cur_raw / 32767.0) * 8.0

    def initialize_motors(self):
        """Initialize both motors with proper drive mode - FROM OLD CODE (EXACT PRESERVATION)"""
    ...
    def emergency_stop(self):
        """Emergency stop both motors - FROM OLD CODE (EXACT PRESERVATION)"""
