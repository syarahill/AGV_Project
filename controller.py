...
class Controller:
    ...
    def _control_loop(self):
        ...
        while self.running:
            ...
            try:
                # ENHANCED: UWB Navigation logic
                if self.state == AGVState.UWB_NAVIGATION and self.target_position:
                    ...
                    # Calculate required heading (0° = North, 90° = East)
                    required_heading = math.degrees(math.atan2(dx, dy))
                    if required_heading < 0:
                        required_heading += 360
                    
                    # Calculate heading difference
                    heading_diff = (required_heading - self.current_heading) % 360
                    if heading_diff > 180:
                        heading_diff -= 360
                    
                    # Check if needs turning
                    if abs(heading_diff) > ROTATION_THRESHOLD:
                        # Turn in place
                        turn_direction = 1 if heading_diff > 0 else -1
                        self.desired_rpm = [
                            -turn_direction * NAVIGATION_TURN_SPEED,
                            -turn_direction * NAVIGATION_TURN_SPEED
                        ]
                    else:
                        # Move forward
                        ...
                        self.desired_rpm = [
                            -NAVIGATION_MOVE_SPEED,
                            NAVIGATION_MOVE_SPEED
                        ]
                    
                    # Timeout check
                    if current_time - self.navigation_start_time > 30:  # 30-second timeout
                        logger.warning("Navigation timeout - stopping")
                        self.desired_rpm = [0, 0]
                        self.state = AGVState.IDLE
